...
export const RESOURCE_TYPES = [
  ...
  // LABEL ONLY. The stored value stays `lodging` — it is in the CHECK
  // constraint and on every record. Renaming a column value to change a word
  // on screen would be a migration in exchange for nothing, and the schema is
  // free to use a broader term than the interface does.
  { key: "lodging", label: "Hotel", plural: "Hotels" },
 ...
 */
/**
 * Filters that only mean something for a ballpark.
 *
 * County joins surface and amenity here: it is how this product navigates a
 * softball directory, not a useful way to find a hotel. State is the one
 * geographic filter that reads sensibly for every type.
 */
export const FACILITY_ONLY_FILTERS = ["county", "surface", "amenity"];
