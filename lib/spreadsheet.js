...
function loadSheetJs() {
  ...
  sheetJsPromise = new Promise((resolve, reject) => {
    ...
    tag.src = SHEETJS;
    tag.async = true;
    tag.onload = () => (window.XLSX ? resolve(window.XLSX) : reject(new Error("load failed")));
...
export async function readSpreadsheet(file) {
  ...
  if (name.endsWith(".xlsx") || name.endsWith(".xls")) {
    ...
    try {
      ...
      return {
        error:
          "Excel support couldn't load. Save the file as CSV in Excel and upload that instead.",
      };
    }
