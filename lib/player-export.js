...
/** MM/DD/YYYY — what the product shows, and what the importer reads back. */
const usDate = (v) => {
  const s = v === null || v === undefined ? "" : String(v).trim().slice(0, 10);
...
export function exportColumns(rows = []) {
  ...
  const cols = [
    // COACH-FACING, not a schema dump.
    //
    // One name column. legal_first_name / preferred_first_name / last_name are
    // internal storage: a player only ever has them because her import file
    // happened to use separate columns, and no player anywhere has a preferred
    // name that differs from her legal one — so exporting all four leaked
    // database shape without carrying any information the coach could not
    // already read in "Player Name".
    //
    // Storage is untouched. If a player ever does have a different preferred
    // name, the drawer still shows her legal name, and this column keeps
    // showing what she is called, which is what a roster is for.
    "Player Name",
    "Type",
...
export function exportRow(row, shape) {
  ...
  const out = [
    // The name she is called. composeFullName() already prefers a "goes by"
    // name where one exists, and full_name is kept derived from it, so this is
    // the display name without needing to recompute anything here.
    text(p.full_name),
    ...
    text(p.grad_year), usDate(p.date_of_birth), text(p.high_school),
