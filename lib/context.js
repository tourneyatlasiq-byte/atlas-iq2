...
export const getContext = cache(async function getContext() {
  ...
  if (!profile?.organization_id) {
    return { user, profile: profile ?? null, organization: null, teams: [], team: null, seasons: [], season: null, features: {} };
  }
  ...
  /**
   * Premium feature flags for this organization.
   *
   * Presentation only. The database remains the authoritative gate: every QAB
   * policy begins with auth_org_has_feature('qab'), so a disabled organization
   * reads zero rows no matter what this object says. This exists so navigation
   * and copy can be honest about what the organization has, which RLS alone
   * cannot express — the app could previously only see emptiness.
   *
   * Fails closed. getContext() renders every authenticated page, so an error
   * here must degrade to "no premium features", never to a thrown page.
   */
  let features = {};
  try {
