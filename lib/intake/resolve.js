...
export function diffRecord(incoming = {}, existing = {}) {
  ...
  for (const [key, value] of Object.entries(incoming)) {
    ...
    // derivedOnly fields are not importable from a column, but they are
    // written when another field produces them — person_type sets the staff
    // role label as a pair.
    if (!field || !field.destination) continue;
    if (!field.importable && !field.derivedOnly) continue;
