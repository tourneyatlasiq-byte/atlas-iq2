...
export function suggestMappings(headers = []) {
  ...
  for (const header of headers) {
    ...
    mappings.push({
      ...
      // The registry has always known full_name is required for intake; the
      // mapping never carried it, so the UI offered it as an ordinary
      // checkbox. Unchecking it was permitted and every row then failed one
      // step later as "row has no player name" — an allowed action punished
      // afterwards, with a message about rows rather than the control tapped.
      required: Boolean(field.requiredForIntake),
      pendingMigration: field.pendingMigration,
      // A BEST GUESS IS A SUGGESTION, NOT AN AUTHORISATION.
      //
      // Only an exactly recognised header switches itself on. A probable match
      // is still shown, with its suggested destination, but the coach has to
      // tick it before it can write anything. "Status" once fuzzy-matched the
      // address field `state`, and a coach re-importing their own export would
      // have written "Active" into every player's State. The suggestion is
      // useful; acting on it unasked is not.
      //
      // Opt-in is a separate, stricter gate. Sensitive is only a LABEL: a
      // parent email is sensitive and still auto-enables, because importing it
      // is the reason the column was mapped.
      autoEnabled: !field.optIn && (SYNONYM_INDEX.has(base) || SYNONYM_INDEX.has(raw)),
      optIn: field.optIn,
