...
export function buildRowPlan({
   ...
   * "04/03/2010" is 4 March or 3 April depending on where the file came from,
   * and guessing a minor's date of birth is not acceptable — so the row stops
   * here, naming the value the coach supplied, rather than importing a blank
   * and letting readiness report the player as missing one.
   */
  for (const u of row._unreadable ?? []) {
    review.push(`${u.label} "${u.raw}" could be read more than one way — `
              + `use a format like 2010-03-04 or 4 Mar 2010`);
  }
