...
import { composeFullName, parseXHandle, parsePersonType } from "./normalize.js";
...
export function buildRowPlan({
   ...
   *
   * players carries flat parent_name / parent_email / parent_phone today, and
   * they hold exactly one contact. The first contact therefore writes there
   * and works now; a second cannot, and waits for player_contacts rather than
   * being concatenated into notes or any other field.
   *
   * Migration A moves contact storage to player_contacts, at which point the
   * first contact joins the others and this branch is removed.
   */
  ...
  contacts.forEach((c, n) => {
    if (n === 0) {
      const flat = {};
      if (c.full_name) flat.parent_name = c.full_name;
      if (c.email) flat.parent_email = c.email;
      if (c.phone) flat.parent_phone = c.phone;
      if (Object.keys(flat).length) {
        const pw = writes.find((w) => w.table === "players");
        if (pw) Object.assign(pw.values, flat);
        else writes.push({ table: "players", op: target ? "update" : "insert",
                           targetId: target?.id ?? null, values: flat });
      }
      return;
    }
    writes.push({
    ...
    pendingKeys.push("additional contacts");
  });
