...
import { composeFullName, parseXHandle, parsePersonType, classifyContactMethod, classifyDate } from "./normalize.js";
...
export function buildRowPlan({
  ...
  const linkWrites = [];
  // { X: { handle, url } } — assembled from however many columns describe it.
  const linkParts = {};
  const review = [];
  ...
  for (const [key, value] of Object.entries(row)) {
    ...
    if (field.level === "link") {
      // Collected, not written yet. X Handle and X URL are two columns
      // describing ONE link — a label and an address — so they are combined
      // after the loop. Writing each as it is met produced two link records,
      // or (because both once mapped to the same key) let the URL column
      // overwrite the handle and store the address as the label.
      if (value === null || value === undefined || value === "") continue;
      linkParts[field.linkType] = linkParts[field.linkType] ?? {};
      linkParts[field.linkType][field.key === "social_url" ? "url" : "handle"] = value;
      continue;
    }
  ...
  /**
   * ONE LINK PER TYPE, built from whatever the file supplied.
   *
   * url  — the address to store. Taken from an explicit URL column when there
   *        is one, otherwise composed from the handle.
   * label— what the coach calls it. The handle when they gave one; otherwise
   *        the handle parsed back out of the URL, because "@bellaramos" is a
   *        better thing to show than the full address.
   *
   * A URL column alone is enough. A handle alone is enough, because
   * parseXHandle() already composes the address deterministically — no data is
   * invented. Anything unreadable is surfaced rather than guessed at.
   */
  for (const [linkType, part] of Object.entries(linkParts)) {
    const fromUrl = part.url ? parseXHandle(part.url) : null;
    const fromHandle = part.handle ? parseXHandle(part.handle) : null;

    if (part.url && !fromUrl) {
      review.push(`X URL: "${part.url}" isn't a recognisable X address`);
      continue;
    }
    if (part.handle && !fromHandle) {
      review.push(`X handle: "${part.handle}" isn't a recognisable X handle`);
      continue;
    }

    const url = fromUrl?.url ?? fromHandle?.url;
    if (!url) continue;

    // The coach's own handle wins as the label. With only a URL column, the
    // handle is read back out of the address — "@bellaramos" is what a coach
    // recognises, and storing the full address as the label is the
    // degradation this change exists to stop.
    const handleFromUrl = url.replace(/^https?:\/\/(www\.)?(x|twitter)\.com\//i, "")
      .replace(/[/?#].*$/, "");
    const label = fromHandle?.label
      ?? (handleFromUrl ? `@${handleFromUrl}` : null);

    linkWrites.push({
      table: "player_links",
      op: "insert",
      linkType,
      values: { link_type: linkType, url, label },
    });
  }
  ...
  /**
   * A date we could not read is not the same as one that was not given.
   * Slash dates are read as MM/DD/YYYY, the format Season Tempo displays, so
   * 05/02/2010 is 2 May and never 5 February. Anything outside the accepted
   * formats, or an impossible date like 02/30, stops the row here naming the
   * value the coach supplied — rather than importing a blank and letting
   * readiness report the player as missing a date of birth.
   */
  for (const u of row._unreadable ?? []) {
    // Suggest the format the product itself displays. Telling a coach to type
    // the database's internal representation is asking them to work around us.
    review.push(`${u.label} "${u.raw}" isn't a date we can read — `
              + `use MM/DD/YYYY, like 05/02/2010`);
  }
