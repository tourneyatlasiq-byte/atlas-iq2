...
/**
 * One canonical candidate shape.
 *
 * `contacts` is normalised here because the two sources spell it differently:
 * a PostgREST embed arrives as `player_contacts`, the drawer resolves it as
 * `contacts`. matchPlayer() reads `candidate.contacts`, so fetching the rows
 * is not enough on its own — they have to arrive under the name it reads.
 */
export function toCandidate(player = {}) {
  ...
  return {
    ...
    contacts: (contacts ?? []).map((c) => ({ id: c.id ?? null, email: c.email ?? null })),
  };
}
