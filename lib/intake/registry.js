...
/**
 * THE position vocabulary. Every surface reads this list.
 *
 * It was duplicated three times — here, in lib/queries/roster.js, and again
 * inside RosterClient — so adding a position meant finding all three or
 * shipping a roster chip the importer would reject. The other two now derive
 * from this one.
 *
 * MIF, CIF and OF are GROUPED designations a coach uses when a player covers
 * an area rather than one bag. They are additions, not replacements: SS, 2B
 * and the rest stay, because a player who only plays shortstop should still be
 * recorded that way.
 */
...
export const FIELDS = [
  ...
  f({
    ...
    category: "core", destination: "players.legal_first_name",
  ...
  f({
    ...
    category: "core", destination: "players.preferred_first_name",
  ...
  f({
    ...
    category: "core", destination: "players.last_name",
  ...
  f({
    ...
    // NOT optIn. Readiness calls a missing date of birth the highest-priority
    // gap ("sanctioning bodies require it to roster a team") and the matcher
    // uses it as its strongest corroborating signal — so gating it behind a
    // checkbox made the product ask for something it then declined to import,
    // and weakened matching at the same time. It stays labelled `sensitive`,
    // so a coach still sees they are importing a minor's identity data; they
    // are simply not required to hunt for a toggle to do the obvious thing.
    key: "date_of_birth", label: "Date of birth", level: "player", type: "date",
    category: "sensitive", sensitive: true,
  ...
  // ---- player roster ----------------------------------------------------
  f({
    key: "bats", label: "Bats", level: "player", type: "hand",
    category: "roster", destination: "players.bats",
    synonyms: ["bats", "batting", "hits"],
  }),
  f({
    key: "throws", label: "Throws", level: "player", type: "throwing_hand",
  ...
  f({
    ...
    category: "recruiting", destination: "players.high_school",
    synonyms: ["high school", "school", "hs", "current school"],
  }),

  // ---- mailing address --------------------------------------------------
  // Colleges mail recruiting material to the player, so this is player-level.
  // PLANNING FIELDS ONLY. Address is never matching evidence: two families
  // sharing an address are not the same child, and a move is not a new person.
  // MATCH_EVIDENCE is an explicit list in match.js and is untouched by these.
  f({
    key: "street_address", label: "Address line 1", level: "player", type: "text",
    category: "sensitive", sensitive: true, destination: "players.street_address",
    synonyms: ["address", "street address", "address 1", "address line 1",
               "mailing address", "street", "addr", "home address"],
  }),
  f({
    key: "street_address_2", label: "Address line 2", level: "player", type: "text",
    category: "sensitive", sensitive: true, destination: "players.street_address_2",
    synonyms: ["address 2", "address line 2", "apt", "apartment", "unit", "suite",
               "address2", "apt/unit"],
  }),
  f({
    key: "city", label: "City", level: "player", type: "text",
    category: "sensitive", sensitive: true, destination: "players.city",
    synonyms: ["city", "town"],
  }),
  f({
    key: "state", label: "State", level: "player", type: "text",
    category: "sensitive", sensitive: true, destination: "players.state",
    synonyms: ["state", "province", "state/province", "st"],
  }),
  f({
    key: "zip", label: "ZIP", level: "player", type: "text",
    category: "sensitive", sensitive: true, destination: "players.zip",
    synonyms: ["zip", "zip code", "zipcode", "postal code", "postcode", "post code"],
  }),
  ...
  f({
    ...
    category: "contact", repeatable: true,
  ...
  f({
    ...
    // "contact relationship" is what our own export produces once the group
    // number is stripped, so it is listed explicitly rather than left to a
    // fuzzy match that would now default to Skipped.
    synonyms: ["relationship", "relationship to player", "relation",
               "contact relationship"],
  }),
  f({
    key: "contact_email", label: "Contact email", level: "contact", type: "email",
    category: "contact", sensitive: true, repeatable: true,
  ...
  f({
     ...
     */
    // linkType X, the SAME as social_handle: these two columns describe one
    // link record, not two links. The plan builder groups by linkType.
    key: "social_url", label: "X / Twitter URL", level: "link", type: "url",
    category: "recruiting", linkType: "X", destination: "player_links",
    synonyms: ["x url", "twitter url", "x link", "twitter link", "x profile",
               "twitter profile", "social url", "profile url"],
  }),
  f({
  ...
  // OUR OWN EXPORT'S columns, named explicitly so the matcher cannot guess at
  // them. Left unnamed, "Status" fuzzy-matched the address field `state` and
  // "College Interest 1" did the same — a coach re-importing their own export
  // would have written "Active" into a player's State. Naming them here means
  // the column is recognised and deliberately not imported, which is also a
  // clearer thing for a coach to read than a wrong guess.
  f({
    key: "_ignore_status", label: "Status (export only)", level: "season", type: "text",
    category: "ignored", importable: false, collectable: false, destination: null,
    synonyms: ["status", "active", "active/inactive", "roster status"],
  }),
  f({
    key: "_ignore_joined_date", label: "Joined date (export only)", level: "season", type: "date",
    category: "ignored", importable: false, collectable: false, destination: null,
    synonyms: ["joined date", "joined", "date joined"],
  }),
  f({
    key: "_ignore_role_label", label: "Role label (export only)", level: "player", type: "text",
    category: "ignored", importable: false, collectable: false, destination: null,
    synonyms: ["role label", "role"],
  }),
  f({
    key: "_ignore_college_interest", label: "College interest (export only)",
    level: "player", type: "text", repeatable: true,
    category: "ignored", importable: false, collectable: false, destination: null,
    synonyms: ["college interest", "college interests", "college"],
  }),
  f({
    key: "_ignore_college_interest_notes", label: "College interest notes (export only)",
    level: "player", type: "text", repeatable: true,
    category: "ignored", importable: false, collectable: false, destination: null,
    synonyms: ["college interest notes", "college notes"],
  }),
  f({
    key: "_ignore_contact_primary", label: "Contact primary (export only)",
    level: "contact", type: "text", repeatable: true,
    category: "ignored", importable: false, collectable: false, destination: null,
    synonyms: ["primary", "contact primary", "is primary"],
  }),
  f({
...
/**
 * Destinations awaiting a migration.
 *
 * Empty since Migrations A (player_contacts) and B (structured names) landed.
 * Retained because the planner still refuses to execute a plan containing one,
 * so a future field can be added as pending without reworking the guard.
 */
...
/**
 * Every players column planning can compare or write.
 *
 * DERIVED FROM THE REGISTRY, not hand-listed. A hand-written select is how the
 * server ended up planning against a partial record: seven columns the diff
 * compares were never fetched, so an incoming value for any of them looked
 * like a FILL against an absent stored value instead of a CONFLICT against a
 * real one — a silent overwrite with no decision shown to the coach.
 *
 * Adding an importable player field to the registry now extends this
 * automatically, and a contract test fails if the server query drifts from it.
 */
export function planningPlayerColumns() {
  const cols = new Set(["id", "organization_id"]);
  for (const f of writableFields()) {
    if (f.level !== "player" || !f.destination?.startsWith("players.")) continue;
    cols.add(f.destination.slice("players.".length));
  }
  // Written by intake_apply alongside person_type, and read back by the
  // drawer, but not itself an importable field.
  cols.add("other_role_label");
  // Legacy contact column: still a matching source for un-backfilled records.
  cols.add("parent_email");
  return [...cols].sort();
}
