...
export async function updateRosterMember(formData) {
  try {
    ...
    // high_school is edit-only. The add path goes through roster_add_member(),
    // which has no high_school parameter, so a value typed on the Add form
    // would be accepted by the UI and silently dropped by the RPC. Giving it a
    // parameter needs a migration, which is out of scope here.
    //
