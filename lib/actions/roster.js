...
export async function addRosterMember(formData) {
  try {
    ...
    if (assignError) {
      // The player row CANNOT be cleaned up here: `players` has no DELETE
      // policy (#177), so this delete matches zero rows and raises nothing.
      // It is attempted because the policy could change, but its result is
      // now checked rather than assumed, and the person is only described as
      // cleaned up when the row is verifiably gone.
      //
      // C3c replaces this pattern outright with an atomic RPC, where all three
      // inserts share one transaction and there is nothing to compensate for.
      ...
      const orphaned = (cleaned ?? []).length === 0;
      if (orphaned) {
        console.error(
          `addRosterMember: assignment failed and the player record could not be removed. ` +
          `Player ${created.id} exists with no season assignment.`
        );
      }
      return {
        ok: false,
        error: assignError.message,
        playerRecordLeftBehind: orphaned,
      };
    }
