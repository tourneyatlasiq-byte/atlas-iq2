...
function friendly(error, context = {}) {
  ...
  if (m.includes("read-only") || m.includes("finished")) {
    return "That season has finished, so its event roster can't be changed.";
  }
  if (error?.code === "23505" || m.includes("duplicate key")) {
    return `${context.name ?? "That player"} is already on this event roster.`;
  }
  ...
  return "We couldn't update the event roster. Please try again.";
}

async function guard() {
  const ctx = await requireSeasonContext();
  if (!canWrite(ctx.profile)) {
    throw new Error("Your role doesn't allow changes to the event roster.");
  }
  return ctx;
}
