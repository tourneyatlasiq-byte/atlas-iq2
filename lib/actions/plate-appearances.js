...
export async function recordPlateAppearance({
  ...
  try {
    ...
    // Out-of-range is dropped rather than rejected: an unknown position is a
    // null, and a bad one must never cost a coach a recorded at-bat mid-game.
    const slot =
      Number.isInteger(battingOrder) && battingOrder >= 1 && battingOrder <= 40
        ? battingOrder
        : null;
      ...
      .upsert(
        {
          ...
          player_id: playerId,
          pa_number: paNumber,
          // Snapshot of where this batter was in the order at tap time. Sent
          // from the client because only the client knows the order that was
          // on screen when the tap happened — which is what makes the value
          // correct after an offline queue is replayed.
          batting_order: slot,
          inning,
          qab_reasons: normalizeReasons(reasons),
          notes,
          // recorded_by is deliberately omitted. enforce_pa_attribution()
          // assigns it from auth.uid() and ignores anything sent, so an entry
          // can never be attributed to someone else.
        },
        { onConflict: "id", ignoreDuplicates: true }
      )
      .select("id, pa_number, batting_order, qab_reasons, is_qab, updated_at");
...
export async function correctPlateAppearance({
  ...
  try {
    ...
    // batting_order is deliberately absent from every field this can set.
    // enforce_pa_attribution() also restores it from OLD, so a correction
    // cannot rewrite it even if a caller sends one.
    const fields = {};
...
export async function voidPlateAppearance({ id, expectedUpdatedAt = null }) {
  try {
    ...
    // voided_by is deliberately not sent. enforce_pa_attribution() assigns it
    // from auth.uid() on the active->voided transition, so the database is the
    // only writer and a direct PostgREST call cannot forge it either.
    let q = supabase
      .from("plate_appearances")
...
export async function restorePlateAppearance({ id }) {
  try {
    ...
    // Clearing voided_at is enough — enforce_pa_attribution() clears voided_by
    // whenever the row comes back to active, so the void attribution cannot
    // outlive the void.
    const { data, error } = await supabase
      .from("plate_appearances")
