...
export async function updatePlayerContact(formData) {
  try {
    ...
    if (contactId === null) {
      ...
      // Whole resolved contact first, submitted edits second.
      const merged = {
        full_name: fields.full_name ?? legacy.full_name,
        relationship: fields.relationship ?? legacy.relationship,
        email: fields.email ?? legacy.email,
        phone: fields.phone ?? legacy.phone,
        preferred_method: fields.preferred_method ?? legacy.preferred_method,
      };

      if (!hasDetail(merged)) {
        return { ok: false, error: "A contact needs a name, relationship, email or phone number." };
      }
    ...
    if (!hasDetail(next)) {
      return {
        ok: false,
        error: "A contact needs a name, relationship, email or phone number. Use Remove contact to delete it.",
      };
    }
...
/**
 * Makes one contact primary.
 *
 * Demote-then-promote, in that order. player_contacts_one_primary is a plain
 * unique index checked per row, so promoting first could transiently leave two
 * primaries and fail on row order alone. These are two statements rather than
 * one transaction; if the second fails the player is left with no explicit
 * primary, which C3a already handles deterministically. That degrades to a
 * display default rather than to a broken record.
 */
export async function setPrimaryContact(formData) {
  try {
    ...
    if ((promoted ?? []).length === 0) {
      return { ok: false, error: "That contact could not be made primary." };
    }
