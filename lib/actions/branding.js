...
// Raster only. SVG is omitted deliberately: the bucket is publicly readable
// and an SVG can carry script, so it is not worth the surface for a logo.
const TYPES = { "image/png": "png", "image/jpeg": "jpg", "image/webp": "webp" };

/**
 * Owner of an organization — no season involved.
 *
 * requireSeasonContext() throws on a past season, so an owner reviewing last
 * year could not change the logo and was told the season was read-only.
 * Branding belongs to the organization, not to a season, so the season gate
 * never applied here.
 *
 * Owner authorization and organization scoping are unchanged, and the storage
 * policy enforces both independently.
 */
async function ownerOnly() {
  ...
  if (ctx.profile?.role !== "owner") {
    throw new Error("Only an owner can change the organization logo.");
  }
