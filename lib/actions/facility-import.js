...
import { requireSeasonContext, isOrgAdmin } from "../context";
...
export async function importFacilities(payload) {
  try {
    ...
    // Admin only. One click here writes hundreds of rows into the shared Atlas
    // directory that every organization reads. Adding a single facility by
    // hand remains available to any writer.
    if (!isOrgAdmin(ctx.profile)) {
      throw new Error(
        "Only an owner or admin can import facilities. You can still add a single facility by hand."
      );
    }
    ...
    for (const row of rows) {
      ...
      if (near) {
        duplicates.push({
          ...
          reason: `Looks like ${near.atlas_id} "${near.name}" in the same city. Not created — add it manually if it is genuinely a different facility.`,
        });
        continue;
      }
