 ...
 * Season record.
 *
 * Counts only games that have been played and carry a result. A scheduled
 * game contributes nothing, which is why the record reads 0-0 before the
 * season starts rather than inventing wins from unplayed fixtures.
 */
export function seasonRecord(tournaments, today = new Date()) {
  const iso = today.toISOString().slice(0, 10);
  const played = tournaments
    .flatMap((t) => t.games ?? [])
    .filter((g) => g.result && g.game_date && g.game_date <= iso);
