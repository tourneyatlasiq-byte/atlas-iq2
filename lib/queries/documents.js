...
/**
 * Documents grouped by the entity they're attached to.
 *
 * Used by the Team and Tournament IQ drawers. RLS has already removed anything
 * the caller may not see, so a coach's map simply contains no Birth Certificate
 * entries — there is no count or placeholder to leak.
 */
export async function documentsByEntity(column) {
  const supabase = createClient();

  const { data, error } = await supabase
    .from("documents")
    .select("id, category, file_name, uploaded_at, file_size, mime_type, player_id, tournament_id, facility_id")
    .not(column, "is", null)
    .order("uploaded_at", { ascending: false });
