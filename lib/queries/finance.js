...
import {
  isActual, isCommittedUnpaid, budgetLineFinance, toCents, sumMoney, incomeCategoryBucket,
} from "../finance-rules";
...
export async function listPlayerPayments(seasonId) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      ...
      `id, initial_cost, player_id, exempt, player_name,
  ...
  return (data ?? [])
    .map((p) => {
      ...
      return {
        ...
        balance,
        /**
         * EXEMPT IS ITS OWN STATUS.
         *
         * A player the organization decided not to charge has a balance of
         * zero, and `balance <= 0` would report them as "Paid in Full" — a
         * claim that money was received. Nothing was received; nothing was
         * asked for.
         */
        status: p.exempt
          ? "No dues"
          : balance <= 0 ? "Paid in Full" : paid > 0 ? "Partial" : "Not Started",
...
export function fundsIn(transactions, payments, budgetItems = []) {
  ...
  // Classified, not string-matched. Free-text categories meant an exact match
  // on "Fundraising"/"Sponsors" missed a real combined category.
  const txnCategory = (t) => t.budget_item?.category ?? t.category;
  const byCategory = (bucket) =>
    sumMoney(
      income.filter((t) => incomeCategoryBucket(txnCategory(t)) === bucket).map((t) => t.actual_amount)
    );
  ...
  const goalFor = (bucket) =>
    sumMoney(
      budgetItems
        .filter((b) => b.is_income && incomeCategoryBucket(b.category) === bucket)
        .map((b) => b.budgeted ?? 0)
    );
