...
export async function seasonBudgetReport(seasonId, { variant = "planned" } = {}) {
  ...
  if (profile.status === "none" && profile.unlinked > 0) {
    ...
    warnings.push({
      blocking: true,
      title: "Some players have no dues set",
      message:
        `Dues are set for ${profile.withDues} of ${profile.activeRosterCount} active players. ` +
        `${profile.missingCount === 1 ? "One player still needs" : `${profile.missingCount} players still need`} ` +
        "dues assigned before this report can go to families. Set them in Finance → Player Dues.",
    });
  }
  ...
  /* Coach-facing notices. Never rendered in the document, and never blocking:
     the report's figures are already scoped to the active roster, so neither
     condition affects a single number a parent will read. */
  if (profile.inactiveWithDues > 0) {
    warnings.push({
      blocking: false,
      title: "Dues records for players no longer on the roster",
      message:
        `${profile.inactiveWithDues} dues ${profile.inactiveWithDues === 1 ? "record belongs" : "records belong"} to a player ` +
        "who has left the active roster. Those records are untouched and still count in Finance, " +
        "but they are excluded from this report, which describes the current roster only.",
    });
  }

  if (profile.unlinked > 0 && profile.identityComplete) {
    warnings.push({
      blocking: false,
      title: "Dues records not linked to a player",
      message:
        `${profile.unlinked} dues ${profile.unlinked === 1 ? "record isn't" : "records aren't"} linked to a player. ` +
        "Every active player has dues set, so this report is accurate, but those records are worth " +
        "checking in Finance → Player Dues.",
    });
  }
