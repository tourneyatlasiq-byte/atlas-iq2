 ...
 *
 * Deliberately absent:
 *   - playerId, so there is no key to filter this document down to one player
 *   - the per-player history ARRAY, its per-game rows, trend and recent form.
 *     Only its LENGTH travels, as a games count. A count is not a history: it
 *     names no opponent, no date and no per-game figure, so it cannot be
 *     reassembled into the parent-facing player report this payload must
 *     never become.
...
export async function qabPerformanceReport(seasonId) {
  ...
  return {
     ...
     *
     * `games` is the LENGTH of the history getSeasonPerformance already built
     * with playerGameHistory() — the games in which this player actually
     * recorded a plate appearance. It is read, never recomputed: no second
     * query, no counting of rows here, so it cannot drift from Performance.
     *
     * Null rather than 0 when no history is present. A player with 14 PA and
     * no history array has an unknown games count, and printing 0 beside 14 PA
     * would state something false. The report renders null as a dash.
     */
    players: (season.players ?? [])
      .map((p) => ({
        name: p.name,
        games: Array.isArray(p.history) ? p.history.length : null,
        qab: p.qab,
        pa: p.pa,
        qabPct: p.qabPct,
      }))
