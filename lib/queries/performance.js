...
export async function getPerformanceOverview(seasonId) {
  ...
  const { data: games, error: gErr } = await supabase
    ...
    //   game_date, then start_time, then id.
    // start_time is null on most games today, so nullsFirst is stated
    // explicitly — an unscheduled game sorts before a timed one on the same
    // day rather than depending on how the driver treats nulls. id is the
    // final tiebreak purely so the order is STABLE between requests; it is a
...
export async function getSeasonPerformance(seasonId, scope = {}) {
  ...
  const gameRows = [...gameMap.values()]
    ...
    //   game_date, then start_time, then gameId.
    // Date alone left same-day games in whatever order the aggregation
    // happened to produce. start_time is null on most games today, so an
    // unscheduled game sorts before a timed one on the same date; gameId is
    // the final tiebreak so repeated requests agree, and is never presented
    // as chronology.
    .sort(
      (a, b) =>
