 ...
 */
export async function getPerformanceOverview(seasonId) {
  const supabase = createClient();

  if (!seasonId) throw new Error("getPerformanceOverview requires a seasonId.");
  ...
  const { data: games, error: gErr } = await supabase
    ...
    .eq("season_id", seasonId)
    .eq("tournament_id", tournament.id)
    // Chronological order, defined rather than inherited:
    //   game_date, then start_time (NULLS LAST), then id.
    //
    // A game with a recorded time can be placed in the day's sequence; a game
    // without one cannot. Sorting the unknown FIRST would assert it happened
    // before the 9:20 game, which is a claim the data does not support.
    // Appending it says only "this one has no recorded time", which is true.
    // NULLS LAST is also Postgres's default for ASC, so this matches the
    // database rather than overriding it.
    //
    // id is the final tiebreak purely so repeated requests agree; it is a
    // random uuid and is never presented as chronology.
    .order("game_date")
...
export async function getSeasonPerformance(seasonId, scope = {}) {
  ...
  if (!seasonId) throw new Error("getSeasonPerformance requires a seasonId.");

  /**
   * SCOPE, EXPLICITLY.
   *
   * This query previously had no filter at all and relied on RLS to decide
   * which rows came back. RLS narrows to seasons the caller may READ — which
   * is every season in their organization, not the one being viewed. With one
   * season per team that happened to be right; the first team to start a
   * second season would have had two seasons of at-bats silently pooled into
   * one "season performance" figure.
   *
   * RLS remains the security boundary and is unchanged. This filter is the
   * application and reporting scope, which is a different question:
   * authorization says what you MAY see, scope says what you ASKED for.
   *
   * The optional narrower scopes exist because a report needs them — a
   * tournament, a game or a player report is the same aggregation over fewer
   * rows. They are applied only when supplied, so nothing speculative is
   * built on top: no new return shape, no report-specific branch.
   */
  let query = supabase
    .from("plate_appearances")
    .select(
      `id, player_id, game_id, is_qab, qab_reasons, batting_order,
       ...
       game:games ( id, game_date, start_time, opponent_name, game_type, result,
  ...
  const gameRows = [...gameMap.values()]
    ...
    // Chronological, not by performance, and deterministic:
    //   game_date, then start_time (games with a time first), then gameId.
    //
    // Date alone left same-day games in whatever order the aggregation
    // happened to produce. A game with no recorded time cannot be placed in
    // the day's sequence, so it sorts after the games that can be — the empty
    // string would otherwise sort it ahead of every real time. gameId is the
    // final tiebreak so repeated requests agree, and is never chronology.
    .sort((a, b) => {
  ...
  // Per-player game history, derived once here rather than in the browser.
  // The Performance component renders already-aggregated data and a report
  // needs the same figures, so both read one implementation.
  const playerRowsWithHistory = playerRows.map((p) => {
    const history = playerGameHistory(gameRows, p.playerId);
    return { ...p, history, recentForm: recentForm(history) };
  });
