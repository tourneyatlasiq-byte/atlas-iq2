...
async function eligiblePlayers(supabase, { tournamentId, seasonId }) {
  ...
  if (pErr) throw new Error(`Could not load the event roster: ${pErr.message}`);
...
/** Does an earlier game in this season have a lineup worth offering to copy? */
export async function hasPreviousLineup(gameId) {
  const supabase = createClient();
  ...
  const { data: candidates } = await supabase
    .from("games")
    .select("id, game_date, opponent_name, lineup:game_lineup_slots ( id )")
    .eq("season_id", game.season_id)
    .lte("game_date", game.game_date)
    .neq("id", gameId)
    // Tie-break MUST mirror copy_previous_lineup()'s
    //   order by g.game_date desc, g.id desc
    // or the card can name one game while the function copies from another.
    .order("game_date", { ascending: false })
    .order("id", { ascending: false });

  // Most recent earlier game in the season that has a lineup. Deliberately not
  // restricted to the same calendar day: Sunday carrying forward Saturday's
  // final order is the common case, and a Friday pool game is still the right
  // starting point on Saturday.
  const prior = (candidates ?? []).find((g) => (g.lineup ?? []).length > 0);
  return prior
    ? {
        opponent_name: prior.opponent_name,
        game_date: prior.game_date,
        batters: (prior.lineup ?? []).length,
      }
    : false;
}
