...
export async function getLineupContext(gameId) {
  ...
  const jerseyByPlayer = new Map(players.map((p) => [p.player_id, p.jersey_number]));

  const lineup = (slots ?? []).map((s) => ({
    player_id: s.player_id,
    full_name: s.player?.full_name ?? "Unknown player",
    jersey_number: jerseyByPlayer.get(s.player_id) ?? null,
...
export async function hasPreviousLineup(gameId) {
  ...
  return prior ? { opponent_name: prior.opponent_name, game_date: prior.game_date } : false;
}
