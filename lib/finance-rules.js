 ...
 */
export function reconcileDues(activePlayers = [], payments = [], membershipIds = null) {
   ...
   */
  /**
   * A THIRD kind, added after a real defect.
   *
   * A dues record can point at a player who has no membership in this season
   * at all — not inactive, not former, simply absent. That happened when
   * removing someone from the roster deleted the membership and left the dues
   * behind, and reporting then presented the leftover as an "inactive player"
   * who had never been inactive.
   *
   * It is not a business state. It is a record that should not exist, so it is
   * labelled "orphaned" and counted separately, where it can be seen and
   * fixed rather than quietly folded into a roster figure.
   *
   * `membershipIds` is optional: callers that do not pass it get the previous
   * two-way classification, unchanged.
   */
  const knownIds = membershipIds ? new Set(membershipIds) : null;
  ...
  const former = (payments ?? [])
    ...
    .map((p) => ({
      ...
      kind: !p.player_id
        ? "unlinked"
        : knownIds && !knownIds.has(p.player_id)
          ? "orphaned"
          : "former",
    }))
    .sort((a, b) => (a.name ?? "\uffff").localeCompare(b.name ?? "\uffff"));
  ...
  return {
    ...
    counts: {
      ...
      /**
       * Never a business figure. A non-zero value means a dues record
       * survived without a season membership, which is a data-integrity
       * condition rather than a person.
       */
      orphaned: former.filter((f) => f.kind === "orphaned").length,
    },
  };
}
